...
const EditContentForm = ({ id, onClose, onSuccess }) => {
  ...
  const handleAddTag = async () => {
    ...
    try {
      ...
      setNewTag("");
      fetchTags();
      toast.success(t("edit-content.tags.add-success"));
    } catch (err) {
      console.error("Error adding tag:", err);
      toast.error(t("edit-content.tags.add-failed"));
    }
  };
        ...
        <div className="bg-white rounded-2xl w-full max-w-4xl p-6 shadow-xl relative">
          ...
          <form onSubmit={handleUpdate} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-4 text-center hover:border-blue-400 transition-all duration-200">
                  <input type="file" accept=".jpg,.jpeg,.png,.gif,.pdf,image/*,application/pdf" onChange={handleFileChange} className="hidden" id="file-upload-edit"/>
                  <label htmlFor="file-upload-edit" className="cursor-pointer block">
                    <div className="text-3xl mb-2">📁</div>
                    <p className="text-gray-600 font-medium mb-1">{file ? t("edit-content.file.change") : t("edit-content.file.choose")}</p>
                ...
                {(filePreview || file) && (
                  <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <div className="flex items-center space-x-3">
                      {filePreview ? (
                        <>
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-800 truncate">{file ? file.name : t("edit-content.file.current-image")}</p>
                            <p className="text-xs text-gray-500">{file ? `${(file.size/(1024*1024)).toFixed(2)} MB` : t("edit-content.file.existing-file")}</p>
                          </div>
                        </>
                      ) : (
                        <>
                      ...
                      <button type="button" onClick={() => { setFile(null); setFilePreview(""); }} className="text-red-500 hover:text-red-700 text-lg font-bold">×</button>
                    </div>
                  </div>
                )}
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
              ...
              <button type="submit" disabled={loading} className={`px-6 py-3 rounded-xl text-white font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all duration-200 ${loading ? "bg-gray-400 cursor-not-allowed":"bg-blue-500 hover:bg-blue-600 focus:ring-blue-500"}`}>
